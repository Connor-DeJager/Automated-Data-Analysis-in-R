...
for(sample in unique_samples) {
  for(gene in unique_genes) {
    ...
    # Filter data for the current sample and gene
    temp_data <- data %>%
      filter(Cleaner_Sample_Name == sample) %>%
      filter(Gene == gene)
    
    # Calculate k as half the number of rows in temp_data (rounded down)
    k_value <- floor(nrow(temp_data) / 3)
    
    # Only run Rosner's ESD test if k_value is at least 1
    if (k_value > 0) {
      
      # Run Rosner's test for up to k_value outliers
      rosner_test <- rosnerTest(temp_data$ddCT, k = k_value)
      
      # Print Block for Dx
      #print("ROSNER TEST: ")
      #print(sample)
      #print(temp_data$ddCT)
      #print(rosner_test)
      #print("...")
      #
      ...
      outlier_values <- temp_data$ddCT[outlier_indices]
      
      # Add the outlier rows to the outlier_data dataframe
      outlier_rows <- temp_data %>% filter(ddCT %in% outlier_values)
      outlier_data <- rbind(outlier_data, outlier_rows)
      
      # Filter the outliers out of temp_data
      temp_data <- temp_data %>% filter(!ddCT %in% outlier_values)
    }
    
    # Add remaining data to stat_data
    stat_data <- rbind(stat_data, temp_data)
    
    # Summarize results for this sample and gene
