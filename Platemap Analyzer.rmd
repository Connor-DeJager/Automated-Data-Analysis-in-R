...
observeEvent(input$select_curve_values, {
  ...
  remove_outliersOLD <- function(data) {
  ...
  remove_outliers <- function(data) {
    ...
    data_clean <- data_clean %>% group_modify(~ {
      ...
      # Perform Rosner's ESD test if there are more than 2 data points
      if (nrow(sample_data) > 2) {
        # Calculate k as one-third of the number of data points (rounded down)
        k_value <- floor(nrow(sample_data) / 3)
        
        # Only run Rosner's ESD test if k_value is at least 1
        if (k_value > 0) {
          # Run Rosner's test for up to k_value outliers
          rosner_test <- rosnerTest(sample_data$Value, k = k_value)
          
          # Identify outliers based on Rosner's results
          outlier_indices <- rosner_test$all.stats$Obs.Num[rosner_test$all.stats$Outlier]
          
          if (length(outlier_indices) > 0) {
            outlier_rows <- sample_data[outlier_indices, ]
            outliers <<- rbind(outliers, outlier_rows)
            
            # Remove identified outliers from the sample_data
            sample_data <- sample_data[-outlier_indices, ]
          }
        }
      }
      return(sample_data)
...
output$barChart <- renderPlot({
  req(summary_data(), final_combined_data())
  
  # Get the summary data from the reactive summary_data()
  plot_summary <- summary_data()
  
  # Ensure column names are correct and convert to numeric
  plot_summary <- plot_summary %>%
    mutate(
      Average = as.numeric(Average),
      `Standard Deviation` = as.numeric(`Standard Deviation`)
    )
  
  # Create the bar chart with SD as error bars
  ggplot() +
    # Plot the bar chart from the summary data
    geom_bar(data = plot_summary, aes(x = Sample, y = Average, fill = Sample), 
             stat = "identity", position = position_dodge(), width = 0.8) +
    geom_errorbar(data = plot_summary, aes(x = Sample, ymin = Average - `Standard Deviation`, 
                                           ymax = Average + `Standard Deviation`), 
                  width = 0.2, position = position_dodge(0.8)) +
    # Add statistical comparisons using the raw data
    stat_compare_means(data = final_combined_data(), aes(x = Sample, y = finalValue),
                       method = "t.test", ref.group = input$reference_select, 
                       label = "p.signif", hide.ns = TRUE, size = 7) +
    theme_minimal() +
    ggtitle(input$plot_title) +  
    xlab(input$x_axis_title) +   
    ylab(input$y_axis_title) +
